...
def minimize_func(parameters, *data):
            ...
            if mod_mode:
                ...
                for name in lowerAwayAttrs:  
                    ...
                    if (WORST_MOD == name) and (mod_games[name] == ALL_MOD_GAMES[name]) and ((PLUS_NAME == "") or (mod_games[PLUS_NAME] == ALL_MOD_GAMES[PLUS_NAME])):
                        worst_vals, worst_win_loss, worst_gameids = [], [], []
                        if (PLUS_NAME != ""):
                            for game in vals_by_mod[PLUS_NAME]:
                                worst_vals.append(vals_by_mod[PLUS_NAME][game]["mofo_away"])
                                worst_vals.append(vals_by_mod[PLUS_NAME][game]["mofo_home"])
                                worst_win_loss.append(vals_by_mod[PLUS_NAME][game]["away_win"])
                                worst_win_loss.append(vals_by_mod[PLUS_NAME][game]["home_win"])
                                worst_gameids.append(game)
                                worst_gameids.append(game)                            
                        for game in vals_by_mod[name]:
                            worst_vals.append(vals_by_mod[name][game]["mofo_away"])
                            worst_vals.append(vals_by_mod[name][game]["mofo_home"])
                            worst_win_loss.append(vals_by_mod[name][game]["away_win"])
                            worst_win_loss.append(vals_by_mod[name][game]["home_win"])
                            worst_gameids.append(game)
                            worst_gameids.append(game)                                       
                ...
                if awayMods > 0 and homeMods > 0:
                    ...
                    if unmod_games == ALL_UNMOD_GAMES:
                        worst_vals, worst_win_loss, worst_gameids = [], [], []
                        for game in vals_by_mod["unmod"]:
                            worst_vals.append(vals_by_mod["unmod"][game]["mofo_away"])
                            worst_vals.append(vals_by_mod["unmod"][game]["mofo_home"])
                            worst_win_loss.append(vals_by_mod["unmod"][game]["away_win"])
                            worst_win_loss.append(vals_by_mod["unmod"][game]["home_win"])
                            worst_gameids.append(game)
                            worst_gameids.append(game)                                       
                        sorted_win_loss = [x for _,x in sorted(zip(worst_vals, worst_win_loss))]
                        sorted_gameids = [x for _,x in sorted(zip(worst_vals, worst_gameids))]
    ...
    linear_error, max_linear_error, min_linear_error, max_error_value, min_error_value = 0.0, 0.0, 150.0, 0.0, 0.0
            ...
            if solve_for_ev:
                ...
                for modname in sorted_vals_by_mod:
                    ...
                    if (len(mod_vals) >= 150):
                        ...
                        mod_gameids.clear()                                        
                        new_plusname = ""
                    for game in vals_by_mod[modname]:
                        mod_vals.append(vals_by_mod[modname][game]["mofo_away"])
                        mod_vals.append(vals_by_mod[modname][game]["mofo_home"])
                        mod_win_loss.append(vals_by_mod[modname][game]["away_win"])
                        mod_win_loss.append(vals_by_mod[modname][game]["home_win"])
                        mod_gameids.append(game)
                        mod_gameids.append(game)               
                    if len(mod_vals) < 150:
    ...
    if (linear_fail < BEST_RESULT):
        ...
        if len(win_loss) > 0:
            ...
            detailtext += "\n{:.4f}% Unmodded fail rate, Best {:.4f}%, {} games, error {:.2f}".format(unmod_rate, BEST_UNMOD, unmod_games, linear_by_mod["unmod"])
            for name in mod_rates:
                detailtext += "\n{:.4f}% {} fail rate, Best {:.4f}%, {} games, error {:.2f}".format(mod_rates[name], name, BEST_MOD_RATES[name], mod_games[name], linear_by_mod[name])            
            ...
            detailtext += "\nBest so far - Linear fail {:.2f}, worst mod = {}, {:.2f}, fail rate {:.4f}%".format(linear_fail, WORST_MOD, LAST_BEST, fail_rate * 100.0)
            if solve_for_ev:
                detailtext += "\nNet EV = {:.4f}, web EV = {:.4f}, season EV = {:.4f}, mismatches = {:.4f}, dadbets = {:.4f}".format(expected_val, web_ev, (-1 * BEST_SEASON), mismatches, dadbets)                        
            detailtext += "\nMax linear error {:.4f}% ({:.4f} actual, {:.4f} calculated)".format(max_linear_error, (max_error_value - max_linear_error), max_error_value)            
            detailtext += "\nMin linear error {:.4f}% ({:.4f} actual, {:.4f} calculated)".format(min_linear_error, (min_error_value - min_linear_error), min_error_value)            
            #if len(errors) > 0 and not solve_for_ev:
            #    errors_output = ", ".join(map(str, errors))
            #    detailtext += "\nMajor errors at: " + errors_output
            #elif not solve_for_ev:
            #    detailtext += "\nNo major errors"
            detailtext += "\nFail error points = {:.4f}, Linearity error points = {:.4f}, total = {:.4f}".format(fail_points, linear_points, linear_fail)
