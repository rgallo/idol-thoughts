...
def minimize_k9_unexvar(game, season_team_attrs, team_stat_data, pitcher_stat_data, pitchers, terms, special_cases, mods):
    ...
    away_strikeouts, home_strikeouts = float(
        away_game["strikeouts"]), float(home_game["strikeouts"])
    away_innings, home_innings = float(
        away_game["innings_pitched"]), float(home_game["innings_pitched"])
    away_so9, home_so9 = (away_strikeouts / away_innings) * \
        9.0, (home_strikeouts / home_innings) * 9.0
    try:
        away_k9 = get_k9(awayPitcher, awayTeam, homeTeam, team_stat_data,
                         pitcher_stat_data, terms, {"factors": special_cases})
    except ValueError:
        away_k9 = -100
    try:
        home_k9 = get_k9(awayPitcher, awayTeam, homeTeam, team_stat_data,
                         pitcher_stat_data, terms, {"factors": special_cases})
    except ValueError:
        home_k9 = -100
    if away_k9 > -99:
        unexvar_k9 += (away_so9 - away_k9) ** 2
    if home_k9 > -99:
        unexvar_k9 += (home_so9 - home_k9) ** 2
    return 2, unexvar_k9
...
def main():
    ...
    unexvarargs = (minimize_k9_unexvar, K9_STLAT_LIST, K9_SPECIAL_CASES, [], stat_file_map, game_list, team_attrs,
                   cmd_args.debug, cmd_args.debug2, cmd_args.debug3)
    unexvar = differential_evolution(base_solver.minimize_func, bounds, args=unexvarargs, popsize=15, tol=0.001,
                                     mutation=(0.05, 0.1), workers=-1, maxiter=10)
    ...
    result = differential_evolution(base_solver.minimize_func, bounds, args=args, popsize=15, tol=0.001,
                                    mutation=(0.05, 0.1), init=unexvar, workers=-1, maxiter=10)
